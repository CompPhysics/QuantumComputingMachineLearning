...
% --- begin date ---
\begin{center}
Spring semester 2026, deaadline June 1
\end{center}
% --- end date ---
...
We would like to suggest four different paths (\textbf{select only one of these}). They are

\begin{enumerate}
\item Implementing quantum Fourier Transforms (QFTs), the phase estimation algorithm and if time allows (optional Shor's algorithm).

\item Quantum Machine Learning project, feel free to suggest alternatives to the one suggested here

\item Implementing Quantum Boltzmann machines

\item Study the solution of quantum mechanical eigenvalue problems with (possible) systems from atomic/molecular physics and quantum chemistry using adaptive QPE
...
\subsection*{Alternative one, QFTs}

This variant of the second project follows closely Robert Hundt's text, sections 6.1-6.6

\paragraph{Project 2 a): Basic mathematics of  Quantum Fourier Transform.}
The Quantum Fourier Transform (QFT) has mathematically the same form as a discrete Fourier transform
...
Show that the first equation is defined in terms of a unitary
transformation. Find the matrix elements for such a unitary
transformation for one-, two- and three-qubit.

Explicitely compute the Fourier transform for $N$ qubits with the initial state $\vert 00\dots 0\rangle$.

\paragraph{Project 2 b): More basic mathematics of  Quantum Fourier Transform.}
The Quantum Fourier transform is normally written out in terms of
various controlled-rotational gates $R_k$. Write these in terms of a
decomposition into single qubit and CNOT gates.  Section 6.2.4 of Hundt for the two-qubit case gives an example.

Write thereafter a quantum circuit to perform the inverse QFT.
Discuss also the pros and cons of performing QFTs compared with for example a classical discrete Fourier transform. Hint: here you can think in terms for the number of floating point operations. Furthermore, keep in mind that due to the binary representation, one can achieve only a given precision. 

\paragraph{Project 2 c): Implementing the Quantum Fourier Transform for one, two and three qubits.}
Here we follow Hundt's section 6.2. Write a code (without using Qiskit or pennylane or similar software) which implements the QFT
for one, two and three qubits. Make sure that the inverse equation (unitarity through equation 6.2 in Hundt) is obeyed. 
Compare your code with for example the QFT functionality of Qiskit.  Compare also your QFT code with the results of the explicit matrix-vector multiplications for two- and three-qubits.

\paragraph{Project 2 d): Generalize the code to an arbitrary number of qubits.}
Now we generalize our code to an arbitrary number of qubits. Test your
code against the Fourier transform from 2a) for $N$ qubits with the
initial state $\vert 00\dots 0\rangle$. Test also the code for unitarity.

\paragraph{Project 2 e): Implement the phase estimation algorithm for one and two qubits.}
Write thereafter your own code for the phase estimation algorithm
discussed in section 6.4 of Hundt. Derive first the phase estimation
equations for the one- and two-qubit cases and find the phase
$\lambda$. Write thereafter a code which implements the phase
estimation algorithm for an arbitrary number of qubits in the
register. Extract the phase and discuss the accuracy of the phase
estimation algorithm, see for example Nielsen and Chuang, \textbf{Quantum Computation and Quantum Information}, Oxford, 2000, section 5.2.

\paragraph{Project 2 f): Bringing back the simple Hamiltonians from project 1. This part is optional.}
We will now compute the eigenvalues of the simpler matrices from project 1 using the phase estimation algorithm.
...
The two-qubit Hamiltonian was defined via a $4\times 4$ Hamiltonian matrix.
That system could be thought of as composed of two subsystems
...
Compute the eigenvalues of these two matrices using the phase
estimation algorithm. Compare the results with those from the
Variational Quantum Eigensolver from project 1.  Discuss the results with pros and cons of the two methods.

\paragraph{Project 2 g): This part is optional.}
...
\subsection*{Alternative two, Quantum Machine Learning}
...
as the probability that this qubit is in the $\vert 1\rangle$ state. Make
...
where $f_i = f(x_i;\boldsymbol{\theta})$ for clarity. The only term we do not know how to calculate is $\frac{\partial}{\partial \boldsymbol{\theta}_k}f(x_i;\boldsymbol{\theta})$, but it turns out there is a simple trick to do this, the so-called parameter shift-rule \href{{https://pennylane.ai/qml/glossary/parameter_shift/}}{\nolinkurl{https://pennylane.ai/qml/glossary/parameter_shift/}}. To calculate the derivative of the model output, we need to evaluate the model twice with the respective parameter shifted by a value $\frac{\pi}{2}$ up and down. The two resulting outputs are then put together to yield the derivative
...
\subsection*{Alternative three, Quantum Boltzmann Machines}

This alternative is a more challenging one and deals with the
implementation and study of what are called Variational Quantum
Boltzmann machines.  It requires familiarity with machine learning and
in particular generative models like Boltzmann machines. It is suited
to those you who follow either FYS5429/9429 and/or FYS4411/9411. In
this case you can combine the second project from one of these courses
with the present report for project 2. You can then hand in one single
(and identical report) for both courses.

The aim here is to implement the variational Quantum Boltzmann machine (VQBM) for studies of the Ising model.
The article we base this is on is
\begin{enumerate}
\item C. Zoufal, A. Lucchi, and S. Woerner, \textbf{Variational quantum Boltzmann machines}, Quantum Machine Intelligence, Volume 3, article number 7, (2021), see \href{{https://link.springer.com/article/10.1007/s42484-020-00033-7}}{\nolinkurl{https://link.springer.com/article/10.1007/s42484-020-00033-7}} for full text.
\end{enumerate}

\noindent
For an implementation, see \href{{https://helda.helsinki.fi/server/api/core/bitstreams/eeadc874-f41c-4795-9149-2cd9a3bfe10d/content}}{\nolinkurl{https://helda.helsinki.fi/server/api/core/bitstreams/eeadc874-f41c-4795-9149-2cd9a3bfe10d/content}}. You should restrict the studies here to some few qubits.  

\subsection*{Alternative four, Adaptive VQE}
...
\begin{enumerate}
\item Extend and generalize the Lipkin model to more than $N=4$ particles.

\item Test different initialization of the qubits for the Lipkin model

\item Implement the adaptive VQE and compare with the VQE from project 1, see for example Keran Chen's master thesis at \href{{https://www.duo.uio.no/handle/10852/114006}}{\nolinkurl{https://www.duo.uio.no/handle/10852/114006}} or at \href{{https://github.com/CompPhysics/QuantumComputingMachineLearning/blob/gh-pages/doc/Projects/2025/Project2/Literature/KeranMScthesis.pdf}}{\nolinkurl{https://github.com/CompPhysics/QuantumComputingMachineLearning/blob/gh-pages/doc/Projects/2025/Project2/Literature/KeranMScthesis.pdf}}. See also the article by Grimsley \emph{et al.,} at \href{{https://www.nature.com/articles/s41467-019-10988-2}}{\nolinkurl{https://www.nature.com/articles/s41467-019-10988-2}}

\item Extend eventually your codes to include more general Hamiltonians. Here you would need to implement the so-called Jordan-Wigner transformation (Notes will be added). 

\item Feel free to suggest additional topics. 
\end{enumerate}
...
\begin{enumerate}
\item Robert Hundt, Quantum Computing for Programmers, Cambridge 2022, in particular chapter 6

\item Michael A. Nielsen and Isaac L. Chuang, Quantum Computation and Quantum Information, Cambridge 2000, in particular chapter 5.
\end{enumerate}
